...
meta_d2b$Clinical_Department
meta_d2b<-readRDS("中间结果1121.RDS")
meta_d2b$PMID <- meta_d2b$pmid 
meta_d2b$Participants <-meta_d2b$content
  meta_d2b$Intervention <-meta_d2b$content
  
  meta_d2b$Comparator <-meta_d2b$content
  meta_d2b$Outcomes <-meta_d2b$content
   
  meta_d2b$Literature_Abbreviation <- paste0("PMID:", meta_d2b$PMID)
  if (data.table::is.data.table(meta_db)) {
    meta_db <- as.data.frame(meta_db)
  }
  if (data.table::is.data.table(meta_d2b)) {
    meta_d2b <- as.data.frame(meta_d2b)
  }  
