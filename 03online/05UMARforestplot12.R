keywordSearchUI <- function(id) {
  ...
  departmentHeader <- function(title, subtitle, icon = "🚀") {
    div(class = "department-header",
        h2(paste(icon, title), style = "margin: 0; font-weight: 700; color: white;"),  # 改为白色
        p(subtitle, style = "margin: 5px 0 0 0; opacity: 0.8; font-size: 0.9rem; color: white;")  # 改为白色
    )
  }
  ...
  tagList(
    ...
    fluidRow(
      box(width = 12, status = "primary", solidHeader = TRUE,
          departmentHeader(
            title = "CAMPASS Meta-Analysis",  # 修改标题
            subtitle = "Leveraging UMAR platform for streamlined evidence synthesis and statistical analysis",
            icon = "🚀"
          ),
          style = "border-top: 3px solid #00695C; box-shadow: 0 2px 8px rgba(0,0,0,0.08); background-color: #00695C;"  # 添加背景色以突出白色文字
      )
    ),
    ...
    fluidRow(
      ...
      column(3,
             box(width = 12, status = "info", solidHeader = TRUE,
                 title = tagList(icon("search"), "Search Settings"),
                 collapsible = TRUE, collapsed = FALSE,
                 
                 # 关键词搜索区域
                 box(width = 12, status = "primary", solidHeader = FALSE,
                     title = "Keywords Search", background = "light-blue",
                     ...
                     div(id = ns("keyword_inputs"),
                         div(class = "keyword-row", id = ns("keyword1_row"),
                             textInput(ns("keyword1"), "Keyword 1", "example", 
                     ...
                     fluidRow(
                       column(6, 
                              actionButton(ns("add_keyword"), "Add Keyword", 
                                           icon = icon("plus"), 
                                           class = "btn-success btn-sm",
                                           style = "width: 100%;")
                       ),
                       column(6, 
                              actionButton(ns("remove_keyword"), "Remove Keyword", 
                                           icon = icon("minus"), 
                     ...
                     br(),
                     actionButton(ns("search"), "Search Database", 
                                  class = "btn-primary", 
                                  icon = icon("search"),
                                  style = "width: 100%; background-color: #00695C; border-color: #004D40; color: white;")
                 ),
                 
                 # 搜索选项
                 box(width = 12, status = "primary", solidHeader = FALSE,
                     title = "Search Options", background = "light-blue",
                 ...
                 box(width = 12, status = "warning", solidHeader = FALSE,
                     title = "Forest Plot Customization", background = "yellow",
                 ...
                 # 漏斗图定制
                 box(width = 12, status = "warning", solidHeader = FALSE,
                     title = "Funnel Plot Customization", background = "yellow",
      ...
      # 右侧结果展示区域
      column(9,
             tabBox(width = 12, id = ns("results_tabs"),
                    title = tagList(icon("chart-bar"), "Analysis Results"),
                    ...
                    tabPanel("Search Results",
                             ...
                             fluidRow(
                               box(width = 12, status = "info",
                                   h4(tagList(icon("list"), "Matching Entries")),
                                   uiOutput(ns("data_alert")),
                                   DTOutput(ns("results_table")),
                                   br(),
                                   downloadButton(ns("download_data"), "Download Results", 
                                                  class = "btn-success",
                                                  style = "background-color: #26A69A; border-color: #00897B;")
                               )
                             )
                    ),
                    ...
                    tabPanel("Forest Plot",
                             ...
                             fluidRow(
                               box(width = 12, status = "success",
                                   h4(tagList(icon("project-diagram"), "Forest Plot")),
                                   ...
                                   fluidRow(
                                     column(12, align = "center",
                                            downloadButton(ns("download_plot"), "Download Forest Plot", 
                                                           class = "btn-success",
                                                           style = "background-color: #26A69A; border-color: #00897B;")
                                     )
                                   )
                    ...
                    tabPanel("Funnel Plot",
                             ...
                             fluidRow(
                               box(width = 12, status = "success",
                                   h4(tagList(icon("balance-scale"), "Funnel Plot for Publication Bias")),
                    ...
                    tabPanel("Meta-Regression",
                             ...
                             fluidRow(
                               box(width = 12, status = "warning",
                                   h4(tagList(icon("line-chart"), "Meta-Regression Analysis")),
                                   ...
                                   fluidRow(
                                     ...
                                     column(6,
                                            ...
                                            actionButton(ns("run_regression"), "Run Meta-Regression", 
                                                         class = "btn-info", icon = icon("rocket"),
                                                         style = "width: 100%; background-color: #00ACC1; border-color: #00838F;")
                                     )
                                   ),
                                   fluidRow(
                                     column(6,
                    ...
                    tabPanel("Summary Statistics",
                             ...
                             fluidRow(
                               box(width = 12, status = "info",
                                   h4(tagList(icon("chart-pie"), "Summary of Search Results")),
                    ...
                    tabPanel("Data Info",
                             ...
                             fluidRow(
                               box(width = 12, status = "info",
                                   h4(tagList(icon("info-circle"), "Dataset Information")),
...
keywordSearchServer <- function(id, meta_db) {
  moduleServer(id, function(input, output, session) {
    ...
    filtered_data <- eventReactive(c(input$search, input$keyword1), {
      ...
      # 检查搜索列是否存在于数据中
      available_columns <- search_columns[search_columns %in% names(meta_db)]
      if (length(available_columns) == 0) {
        showNotification("Warning: Selected search columns not found in database", type = "warning")
        return(data.frame())
      }
      ...
      tryCatch({
        if (search_logic == "OR") {
          search_condition <- rep(FALSE, nrow(meta_db))
          for (col in available_columns) {
            for (keyword in keywords) {
              if (keyword != "") {
                # 处理字符型列
                if (is.character(meta_db[[col]]) || is.factor(meta_db[[col]])) {
                  search_condition <- search_condition | 
                    grepl(keyword, as.character(meta_db[[col]]), ignore.case = TRUE)
                } else {
                  # 对于数值型列，转换为字符进行搜索
                  search_condition <- search_condition | 
                    grepl(keyword, as.character(meta_db[[col]]), ignore.case = TRUE)
          ...
          search_condition <- rep(TRUE, nrow(meta_db))
          for (keyword in keywords) {
            if (keyword != "") {
              keyword_matched <- rep(FALSE, nrow(meta_db))
              for (col in available_columns) {
                if (is.character(meta_db[[col]]) || is.factor(meta_db[[col]])) {
        ...
        result <- meta_db[search_condition, ]
        
        # 如果没有找到结果，显示通知
        if (nrow(result) == 0) {
          showNotification("No results found with current search criteria", type = "warning")
        } else {
          showNotification(paste("Found", nrow(result), "matching records"), type = "message")
        }
    ...
    output$forest_plot <- renderPlot({
      ...
      x_buffer <- (x_max - x_min) * 0.3  # 减小缓冲区
      ...
      p <- ggplot(forest_data_prepared, aes(x = Effect_Size, y = y_position)) +
        ...
        geom_errorbarh(aes(xmin = CI_Lower, xmax = CI_Upper), 
                       height = 0.2, size = 0.8, alpha = 0.8, color = "#3498DB") +
        
        # 效应值点
        geom_point(aes(size = Sample_Size_Scale, fill = Effect_Size), 
                   shape = 21, stroke = 1, color = "white") +
        
        # 研究名称
        geom_text(aes(label = Study, 
                      x = x_range[1] + (x_range[2] - x_range[1]) * 0.02), 
                  hjust = 0, size = input$forest_size/3.5, fontface = "bold",
                  color = "#2C3E50") +
        ...
        scale_x_continuous(limits = x_range, 
                           breaks = pretty(x_range, n = 8)) +
        scale_y_continuous(expand = expansion(mult = 0.1)) +
        
        # 标签和主题
        labs(x = "Effect Size with 95% Confidence Interval", y = "", 
             title = "Meta-Analysis Forest Plot",
             subtitle = paste("Includes", nrow(forest_data_prepared), "studies | Points sized by sample size, colored by effect size")) +
        
        theme_minimal(base_size = input$forest_size) +
        theme(
          panel.grid.major.y = element_blank(),
          panel.grid.minor.y = element_blank(),
          panel.grid.major.x = element_line(color = "gray93", size = 0.4),
          ...
          plot.title = element_text(face = "bold", hjust = 0.5, size = input$forest_size * 1.3,
                                    margin = margin(b = 10)),
          plot.subtitle = element_text(hjust = 0.5, size = input$forest_size * 1.0,
                                       color = "gray50", margin = margin(b = 15)),
          legend.position = "bottom",
          legend.box = "horizontal",
          legend.text = element_text(size = input$forest_size * 0.8),
          legend.title = element_text(face = "bold", size = input$forest_size * 0.9),
          plot.background = element_rect(fill = "white", color = NA),
          panel.background = element_rect(fill = "white", color = NA),
          plot.margin = margin(20, 40, 20, 40)  # 增加边距
        )
      ...
      if (input$show_stats) {
        p <- p + 
          geom_text(aes(label = Estimate, 
                        x = x_range[2] - (x_range[2] - x_range[1]) * 0.02), 
                    hjust = 1, size = input$forest_size/3.5, 
                    color = "#7F8C8D", fontface = "plain")
      }
      ...
      if (input$show_diamond && nrow(forest_data_prepared) > 1) {
        p <- p +
          geom_point(data = overall_effect, aes(x = Effect_Size, y = y_position),
                     shape = 23, size = 8, fill = "#8E44AD", color = "white", stroke = 1.5) +
          geom_errorbarh(data = overall_effect, 
                         aes(xmin = CI_Lower, xmax = CI_Upper, y = y_position),
                         height = 0.15, color = "#8E44AD", size = 1.5, alpha = 0.8) +
          annotate("text", x = overall_effect$Effect_Size, y = overall_effect$y_position - 0.3,
                   label = "Overall Effect", size = input$forest_size/3.2, 
                   color = "#8E44AD", fontface = "bold")
      }
      
      # 添加研究数量注释
      p <- p + 
        annotate("text", x = x_range[1], y = max(forest_data_prepared$y_position) + 0.5,
                 label = paste("Total studies:", nrow(forest_data_prepared)),
                 hjust = 0, size = input$forest_size/3.5, color = "#7F8C8D", fontface = "italic")
